...
import { User, Mail, Phone, BookOpen, GraduationCap, Briefcase, Award, Save, Edit2, CheckCircle } from "lucide-react";
...
export default function StudentProfile() {
  ...
  const saveProfile = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem("token");
      const res = await axios.put(`${import.meta.env.VITE_API_URL}/api/users/update-profile`, profile, {
        headers: { Authorization: `Bearer ${token}` }
      });
      
      // Update local cache for name if changed
      if (profile.name !== localStorage.getItem("name")) {
  ...
  return (
    <div className="p-8 lg:p-12 bg-slate-50 dark:bg-slate-950 min-h-full">
      <div className="max-w-5xl mx-auto space-y-8 pb-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-8">
            <div className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-800 space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase text-slate-500 ml-1">Email (Read Only)</label>
                  <div className="relative">
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-800 min-h-full">
    ...
    <div className={`space-y-1.5 ${spanFull ? 'md:col-span-2' : ''}`}>
      ...
      <div className="relative group">
        ...
        <input 
          ...
          className="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 pl-10 pr-4 py-4 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all font-bold text-sm dark:text-white disabled:opacity-70"
        />
      </div>
    </div>
  );
